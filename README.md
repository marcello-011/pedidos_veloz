# 🛒 Pedidos Veloz — Plataforma Cloud-Native de E-commerce

> **Loja Veloz** | Modernização DevOps End-to-End | MVP em 4 semanas

[![CI/CD](https://github.com/lojaveloz/pedidos-veloz/actions/workflows/ci-cd.yaml/badge.svg)](https://github.com/lojaveloz/pedidos-veloz/actions)
...
- [Início Rápido — Ambiente Local](#início-rápido--ambiente-local)
...
A **Pedidos Veloz** é a plataforma de e-commerce da Loja Veloz, refatorada para arquitetura de microsserviços cloud-native. Esta solução resolve os três problemas críticos identificados:

| Problema Atual | Solução Implementada |
|---|---|
| Indisponibilidade em deploys | Rolling Update com `maxUnavailable: 0` |
| Dificuldade de escalar em picos | HPA automático (CPU/memória) + pré-aquecimento manual |
| Baixa rastreabilidade de falhas | OpenTelemetry + Prometheus + Grafana + Loki |

**Referência de mercado:** Este projeto se inspira no [Google Online Boutique](https://github.com/GoogleCloudPlatform/microservices-demo), aplicação de e-commerce de referência do Google para demonstrar Kubernetes, Istio e OpenTelemetry em escala.
...
```
                         ┌─────────────────────────────────────────┐
                         │              Kubernetes Cluster           │
                         │            (Namespace: pedidosveloz)     │
  Internet               │                                           │
      │                  │  ┌───────────┐    ┌──────────────────┐  │
      ▼                  │  │           │    │                  │  │
  [Ingress NGINX]────────┼─▶│ API       │───▶│  Serviço de      │  │
      :443               │  │ Gateway   │    │  Pedidos         │  │
                         │  │ :8080     │    │  :8081           │  │
                         │  └───────────┘    └────────┬─────────┘  │
                         │                            │            │
...
|---|---|---|---|
| `api-gateway` | Python/FastAPI | 8080 | Roteamento, autenticação, rate-limit |
| `pedidos` | Node.js/TypeScript | 8081 | CRUD de pedidos, orquestração de fluxo |
| `pagamentos` | Node.js/TypeScript | 8082 | Integração com gateway externo de pagamento |
| `estoque` | Node.js/TypeScript | 8083 | Reserva e baixa de itens do estoque |
...
git clone https://github.com/lojaveloz/pedidos-veloz.git
cd pedidos-veloz

# 2. Copie e ajuste as variáveis de ambiente
cp .env.example .env

# 3. Suba o stack completo com UM único comando
docker compose up -d
...
# Banco de dados
POSTGRES_USER=veloz
POSTGRES_PASSWORD=veloz_secret
